...
public class MethodTest2 {
	 ...
	 *  
	 * - 호출시 객체 생성할 필요가 없음
	 * - 프로그램 시작시에 정적 메모리 영역에 메소드가 저장되어 있을거이기 떄문.
	...
	public static int method4(int num1) {
		/*
		 * static 메서드안에서 static이 아닌 field멤버가 접근이 불가능하다.
		 * 스태틱 메서드는 프로그램 시작시 정적메모리영역에 들어가게 되는데
		 * 이때 스태틱으로 선언된 필드(전역변수)도 함께 정적메모리 영역에 들어갈것.
		 * 하지만 스태틱으로 선언되지 않은 전역변수는 스태틱 메모리에 올라가는 시점에 사용이 불가능하므로 에러가발생함.
		 * 
		 */
		

		// static 메서드는 static 필드만 사용이 가능하다
		return num1 * count;
	}
