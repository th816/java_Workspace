...
public class ExceptionRun {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * - 컴파일 에러 : 소스코드상의 문법적인 문제로 발생하는 에러 => 소스코드 수정으로 해결(빨간줄로 오류알려줌)
		 * - 런타임 에러 : 소스코드상으로는 문제가 없는데 프로그램 실행 중 발생하는 에러
		 * 				=> 사용자의 잘못일수도 있고 개발자가 예측해야하는데 하지 못한 경우
		 * - 논리 에러 : 소스코드상으로 문제 없고, 실행했을때도 에러가 발생하지 않지만, 의도한대로 결과가 나오지 않는경우
		 * 
		 * 시스템 에러를 제외한 컴파일 에러, 런타임 에러, 논리 에러같은 비교적 덜 심각한 것들을 가지고 작업을함
