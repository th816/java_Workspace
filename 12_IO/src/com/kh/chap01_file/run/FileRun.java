...
public class FileRun {
	...
	public static void main(String[] args) {
		 ...
		 * 입력정치(키보드, 마우스)로 입력받는 과정을 진행하고자 한다면 반드시 외부매체와의 통로를
		 * 만들어야한다. -> 통로 == 스트림
		 * 
		 * 스트림의 특징:
		 * - 단방향 : 입력이면 입력, 출력이면 출력
		 ...
		 * 스트림의 구분
		 * - 통로의 사이즈 :
		 * 		바이트 스트림 : 1byte짜리가 이동할 수 있을정도의 사이즈(좁은통로) => 입력(XXXInputStream)/출력(XXXOutStream)
		 * 		문자 스트림 : 2byte짜리가 이동할 수 있을정도의 사이즈(넓은 통로) => 입력(XXXReader)/출력(XXXWiter)
		 * 
		 * - 외부매체와의 직접적인 연결여부 : 
		 * 		기반스트림 : 외부매체와 직접적으로 연결되는 통로
		 * 		보조스트림 : 기반스트림만으로 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매체와 연결할 수 없음, 즉 단독사용이 불가능)
