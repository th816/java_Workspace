...
public class B_ArrayCopy {
	 ...
	 * - 얕은 복사 : 배열의 주소 값만 복사	
	 * 
	 * - 깊은 복사 : 동일한 새로운 배열을 하나 더 만들어서 실제 내부 값들을 복사
	 * 
	 * 
	 * 
	 */
	...
	public void method2() {
		// 1. for문 방법(수작업)
		// 새로운 배열을 생성한 후 각 인덱스 별로 내부값을 일일이 대입하는 방법.
	...
	public void method4() {
		// 3. Arrays클래스에서 제공하는 copyOf 
		
		int [] origin = {1, 2, 3, 4, 5};
		
		//복사본 배열 = Arrays.copyOf(원본 배열명 , 복사할갯수)
		int [] copy = Arrays.copyOf(origin, 3); // copy 배열의 크기 == 복사할 갯수
		
		for(int i = 0; i<copy.length; i++) {
			System.out.print(copy[i]+ " ");
		}
	/*
	 * 2. System.arraycopy 메소드 : 몇번 인덱스부터 몇개를 어느 위치에 인덱스에
	 * 복사할 것인지 세밀하게 지정가능.
	 * 
	 * 3. Arrays.copyOf 메소드 : 무조건 원본 배열의 0번 인덱스부터 내가 제시한
	 *	갯수만큼 복사 진행됨. 내가 제시한 길이가 원본배열도 크다면 나머지에는 0의값이
	 * 채워서 복사해줌.
	 * 
	 */
	}
	...
	public void method5() {
		...
		//복사본 배열 = 원본배열.clone();
		int[] copy = origin.clone();//인덱스 직접 지정x, 복사할갯수지정x
		// 무조건 원본배열과 동일한 녀석이 변환됨.
