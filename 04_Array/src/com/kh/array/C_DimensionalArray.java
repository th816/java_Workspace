...
public class C_DimensionalArray {
	...
	public void method1() {
		 ...
		 * 출력(arr[2][4]) -> 3행 5열
		 */
		/* 바깥쪽 for문 => 행에 대해 지정(0행 ~ 2행) == (0행 ~ 행의크기-1) == (0행 ~ arr.legth-1)
	...
	public void method2() {
		int[][] arr = new int [3][5];
		// 순서대로 1, 2, 3, 4, ... 15값을 넣어보기
		
		 // arr[0][0] -> 1, arr[0][1] ->2 , arr[2][4] -> 15
		int value = 1;
		for(int i = 0; i < arr.length-1; i++) {
			for(int j=0; j < arr[i].length-1; j++) {
				arr[i][j] = value++;
	...
	public void method4() {
		...
		// 특정 인덱스로 접근하려했기때문에 NullPointerExecptoin
