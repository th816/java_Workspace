...
public class C_Cast {
	 ...
	 * 
	 * 2. 강제(명시적)형변환 => 자동형변환이 되지 않아 내가직접 강제로 형변환을 해야하는경우
	...
	//자동형변환 : 작은바이트의 자료형이 큰바이트의 자료형으로 형변환도니느경우
	public void autoCasting() {
	
		//1. int(4byte) -> double(8yte)
		int i1 = 10;
		double d1 = /*(double)*/ i1; // 자동으로 형변환이 되었다.
									// 컴퓨터 포장용상자에 휴대폰은 담은꼴과 비슷함.
									// 그런데 int는 정수, double은 실수자료형.
		...
		// 아무리 4byte float라고 해도 long형보다 담을수있는 값의 범위가 더 크기때문에
		...
		/*
		 * char의 범위는 0~65xxx : 각 문자마다 고유의 정수값이 정해져있다. 따라서 쌍방향 변환이 가능하다.
		 * 음수값은 오류가난다.
		 * 참고 : 아스키코드표 (0~127, 영문자, 대소문자, 숫자, 특수문자)
		 * 	     유니코드표 (0 ~65xxx, 영어,숫자,특수문자, 한글, 그외 언어)
		 */
		System.out.println((int)'오');
	...
	//강제(명시적)형변호나 : 큰 크기의 자료형을 작은 크기의 자료형으로 형변환 할 경우.
	public void forceCasting() {
		...
		//해결방법 2. 덧샘 연산전에 dNum변수를 미리 int형으로 강제 형변환
		...
		// 해결방법 3. 애초에 결과값을 double형 변에수 담기. (데이터 손실없이 정확한 값이 담김)
		double dSum = iNum + dNum;
		System.out.println("iSum : "+iSum+", iSum :"+iSum2+", dSum : "+dSum);
