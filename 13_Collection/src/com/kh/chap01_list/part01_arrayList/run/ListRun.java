...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 정리해보면
		 * 데이터들이 새로이 추가되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘) 들이 이미 정의되어있는 틀이 있다.
		 * == 컬렉션
		 * => 다량의 데이터들을 관리하고자 할 때 배열 가지고 충분히 사용했었음, 단 배열의 단점들을 보완한것이 컬렉션
		 * 
		 * 배열과 컬렉션의 차이점
		 * - 배열의 단점
		 ...
		 * 방대한 데이터들이 빈번하게 추가, 삭제, 수정 될 것같다. => 컬렉션
		 * 
		 *  컬렉션의 3가지 분류
		 *
		 * List 계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서유지됨(index의 개념이 있음)/ 중복값 허용
		 *				ex) ArrayList, Vector, LinkedList => ArrayList가 제일 많이 쓰임(90%이상)
		 *
		 * Set 계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 x(index 개념이 없음) / 중복값 허용x
		 *				ex) HashSet, TreeSet
		 *
