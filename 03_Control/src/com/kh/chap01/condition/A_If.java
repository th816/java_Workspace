...
public class A_If {
	 ...
	 * 
	 * 조건문
	 * 조건식을통해 참이냐 거짓이냐를 판단해서 그에 해당하는 코드 실행
	 * 
	 * 조건식
	...
	public void method1() {
		...
		if(num > 0) {
			System.out.println("양수입니다."); //양수입니다. 출력을 먼저 적고 if 블럭으로 감싼다.
		}
	...
	public void method3() {
		 ...
		 * } else if (조건식3) {
		 * .. 조건식3이 true라면 실행시킬 코드작성..
		 * } [else { 
		 *	.. 위의 모든조건식의 결과가 false라면 실행할코드
		 * }]
		 */
		if( num > 0) {
			System.out.println("양수입니다");
	...
	public void method5() {
		...
		System.out.print("성별 : ");
		char gerden = sc.nextLine().charAt(0);
		
		String result = "";
		if(gerden == 'm'|| gerden == 'M') {
		 result = "남학생";
		} else if (gerden == 'f'|| gerden == 'F' ) {
			result = "여학생";
		} else {
			System.out.println("잘못입력하셨습니다.");
			//만약에 이 아래 코드를 실행시키고 싶지 않다면?
			// return이라는 녀석을 사용
			return; // 해당 일반메서드를 빠져나감(메소드를 호출했던곳으로 돌아감)
		}
		//xxx님은 xxx이다.
	...
	public void method6() {
		 ...
		 * 그럼 어떻게하냐? 문자열.equals() 라는방법으로 비교를함
		 */
